...
const AdminDashBoard = () => {
  ...
  const GetDshCount = () => {
    ...
    fetch(url)
        ...
        .then((data) => {
            setDshCount({Vol: data.totalVolunteers,
    ...
    <div className="w-screen">
        ...
        <div className="bg-white shadow-lg rounded-lg w-4/5 h-auto px-4 py-4 flex flex-row justify-between divide-x divide-solid divide-gray-400">
            <div className="relative flex-1 flex flex-col gap-2 px-4">
                <label className="text-gray-800 text-base font-semibold tracking-wider">Organizers</label>
                <label className="text-gray-600 text-4xl font-bold">{DshCount.Org}</label>
                
            </div>
            <div className="relative flex-1 flex flex-col gap-2 px-4">
                <label className="text-gray-800 text-base font-semibold tracking-wider">Campaigns</label>
                <label className="text-gray-600 text-4xl font-bold">{DshCount.Camp}</label>
                
            </div>
            <div className="relative flex-1 flex flex-col gap-2 px-4">
                <label className="text-gray-800 text-base font-semibold tracking-wider">Volunteers</label>
                <label className="text-gray-600 text-4xl font-bold">{DshCount.Vol}</label>
        ...
        <div className="w-4/5  bg-white shadow-lg rounded-sm border border-gray-200">
            ...
            <header className="px-5 py-4 border-b border-gray-100">
                <h2 className="font-semibold text-xl text-gray-800">Joined Recently</h2>
            </header>
                              ...
                              JoinedList.map((data, key) => {
                                ...
                                <tr key={key}>
                                    <td className="p-2 whitespace-nowrap">
                                        <div className="text-left">{data.VolunteerName}</div>
                                    </td>

                                    <td className="p-2 whitespace-nowrap">
                                        <div className="text-left font-medium text-green-500">{data.CampaignName}</div>
                                    </td>

                                    <td className="p-2 whitespace-nowrap">
                                        <div className="text-lg text-left">{data.OrganizerName}</div>
                                    </td>
                                </tr>
                           ); }
                        )}
